...
export default function Home() {
  ...
  useEffect(() => {
    try {
      setRecent(JSON.parse(localStorage.getItem('wordpop-recent') || '[]'));
    } catch {}
  }, []);
      ...
      <section className={result ? 'hero compact' : 'hero'}>
        ...
        <form className="searchBox" onSubmit={onSubmit}>
          ...
          <input
            ...
            placeholder="영어 또는 한글을 입력하세요  예) beautiful, 아름다운"
            aria-label="단어 검색"
          />
        ...
        {!result && (
          <div className="quickWords">
            <span>추천 검색</span>
            {starterWords.map((word) => <button key={word} onClick={() => search(word)}>{word}</button>)}
          </div>
        )}
        {!result && recent.length > 0 && (
          <div className="recentRow">
            <span>최근 검색</span>
            {recent.map((word) => <button key={word} onClick={() => search(word)}>{word}</button>)}
          </div>
        )}
      </section>
        ...
        <div className="content">
          <section className="wordCard panel">
            <div>
              ...
              <div className="wordTitleRow">
                <h2>{result.word}</h2>
                <button className="roundSpeak" onClick={() => speak(result.word)} aria-label="단어 발음 듣기">🔊</button>
              </div>
            ...
            <div className="meaningStack">
              {meanings.slice(0, 4).map((item, i) => (
                <div className="meaningItem" key={`${item.partOfSpeech}-${i}`}>
                  <span>{item.partOfSpeech || 'word'}</span>
                  <strong>{item.korean || item.definition}</strong>
                  {item.definition && item.korean && <small>{item.definition}</small>}
                </div>
              ))}
            </div>
          </section>
          ...
          <section className="panel">
            <div className="sectionHead">
              <div><span className="sectionKicker">EXAMPLES</span><h3>예문으로 익히기</h3></div>
            </div>
            <div className="exampleList">
              {(result.examples || []).map((ex, i) => (
                <article className="example" key={i}>
                  <div className="exampleNumber">{String(i + 1).padStart(2, '0')}</div>
                  <div className="exampleText">
                    <p>{ex.en}</p>
                    <span>{ex.ko}</span>
                  </div>
                  <button className="miniSpeak" onClick={() => speak(ex.en)} aria-label="예문 발음 듣기">🔊</button>
                </article>
              ))}
            </div>
          </section>
          ...
          <section className="panel">
            <div className="sectionHead">
              <div><span className="sectionKicker">RELATED WORDS</span><h3>관련 단어</h3></div>
            </div>
            <div className="relatedGrid">
              {(result.related || []).map((item) => (
                <button key={item.word} className="relatedChip" onClick={() => search(item.word)}>
                  <strong>{item.word}</strong><span>{item.ko}</span>
                </button>
              ))}
            </div>
          </section>
            ...
            {songs.length > 0 ? (
              <div className="songGrid">
                {songs.slice(0, 6).map((song) => (
                  <a className="songCard" key={song.id} href={song.youtubeUrl} target="_blank" rel="noreferrer">
                    {song.artwork ? <img src={song.artwork} alt="" /> : <div className="albumFallback">♪</div>}
                    <div><strong>{song.title}</strong><span>{song.artist}</span><small>검색 단어 · {result.word}</small></div>
                    <b>▶</b>
                  </a>
                ))}
              </div>
            ) : (
              <div className="songEmpty">관련 곡을 자동으로 찾지 못했습니다. 위의 YouTube 검색으로 바로 찾아볼 수 있어요.</div>
            )}
